...
abstract class RecyclerViewArrayAdapter<VH: RecyclerView.ViewHolder, T>:
     ...
     * @throws UnsupportedOperationException if the underlying data collection is immutable
     */
    open fun insert(element: T, index: Int) {
        synchronized(mLock) {
            if (mOriginalValues != null) {
     ...
     */
    open fun find(predicate: (T) -> Boolean): T? = mObjects.find(predicate)

    /**
     * Returns the index of the first occurrence of the specified element in the list, or -1 if the specified
     * element is not contained in the list.
     */
    open fun findIndex(predicate: (T) -> Boolean): Int {
    ...
    open fun getItem(position: Int): T? = if (position > -1 && position < mObjects.size) {
        mObjects[position]
    } else {
        null
    }

    open fun getPosition(element: T): Int = mObjects.indexOf(element)
